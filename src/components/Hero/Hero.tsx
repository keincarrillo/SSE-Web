...
const Hero = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(imgRef.current, { opacity: 0, scale: 1.04 }, { opacity: 1, scale: 1, duration: 1.2, ease: 'power3.out', scrollTrigger: { trigger: sectionRef.current, start: 'top 95%' } })

      gsap.timeline({ scrollTrigger: { trigger: sectionRef.current, start: 'top 95%' } })
        .fromTo(tagRef.current, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: 'power3.out' })
        .fromTo(titleRef.current, { y: 50, opacity: 0 }, { y: 0, opacity: 1, duration: 0.9, ease: 'power3.out' }, '-=0.4')
      ...
      gsap.fromTo(barRef.current, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: 'power3.out', scrollTrigger: { trigger: barRef.current, start: 'top 95%' } })
    }, sectionRef)
  ...
  return (
    <section ref={sectionRef} id="inicio" className="relative md:min-h-screen bg-green overflow-hidden flex flex-col scroll-mt-24">
      ...
      <div className="relative z-20 flex-none md:flex-1 flex flex-col items-center justify-start text-center px-6 md:px-10 pt-32 md:pt-32 lg:pt-44">
        ...
        <h1 ref={titleRef} className="display-title text-white mb-5 md:mb-6"
          style={{ fontSize: 'clamp(1.75rem, 4.5vw, 3.5rem)', lineHeight: '1.05' }}>
          Smile <span className="text-gold">Studio</span> Experts
        </h1>

        <p ref={subRef} className="text-white/90 md:max-w-xl mb-2 text-center"
          style={{ fontSize: 'clamp(.9rem, 1.30vw, 1.35rem)', lineHeight: '1.6' }}>
          Somos un equipo enfocado en la estética y salud dental, creando sonrisas naturales y armónicas con tratamientos personalizados y el uso de tecnología y materiales de calidad para lograr resultados que se vean bien y duren en el tiempo.
        </p>

        <div ref={ctaRef} className="flex flex-wrap gap-4 justify-center mt-1 pb-10 md:pb-0">
          <button
            onClick={() => {
              const lenis = window.__lenis
              if (lenis) lenis.scrollTo('#contacto', { offset: 0, duration: 1.8 })
      ...
      <div ref={barRef} className="relative z-20 w-full bg-gold">
        <div className="w-full px-6 py-2 md:py-3 grid grid-cols-3 items-center divide-x-0 md:divide-x-0">
          {STATS.map(s => (
            <div key={s.label} className="flex flex-col items-center justify-center gap-0.5 py-2">
              <span className="display-name text-green-dark font-black text-lg sm:text-2xl md:text-4xl tabular-nums leading-none">
                {s.prefix}<AnimatedNumber target={s.value} active={statsActive} />{s.suffix}
              </span>
              <span className="font-semibold text-white text-xs lg:text-lg tracking-[0.15em] uppercase text-center">{s.label}</span>
            </div>
          ))}
        </div>
      </div>
