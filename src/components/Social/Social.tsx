...
const IconInstagram = ({ size = 40 }: { size?: number }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const IconFacebook = ({ size = 40 }: { size?: number }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
  ...
  return (
    <div className={`${inView ? 'fade-up' : 'hidden-init'}`}>
        ...
        {phones.map((_, i) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              i === current ? 'w-8 bg-gold' : 'w-2 bg-gray-mid'
            }`}
          />
        ))}
      </div>
      ...
      <div className="flex items-center justify-between mt-6 max-w-50 mx-auto">
        <button
          onClick={prev}
          className="w-10 h-10 rounded-full border border-border flex items-center justify-center text-muted hover:bg-gray transition-colors"
          aria-label="Anterior"
        >
          <ChevronLeft />
        </button>
